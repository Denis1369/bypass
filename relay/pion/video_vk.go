...
func (c *VKClient) createPC(config webrtc.Configuration) error {
	...
	pc.OnConnectionStateChange(func(state webrtc.PeerConnectionState) {
		...
		if state == webrtc.PeerConnectionStateDisconnected || state == webrtc.PeerConnectionStateFailed {
			if c.laneBinder != nil {
				c.logFn("vk: resetting lane binder due to state=%s", state.String())
				c.laneBinder.Reset()
			}
		}
		if state == webrtc.PeerConnectionStateConnected && len(c.vp8tunnels) > 0 && !c.tunnelStarted {
			c.logFn("vk: === CONNECTED - starting VP8 tunnel ===")
			for i, track := range c.sampleTracks {
				c.logFn("vk: sampleTrack[%d] id=%s kind=%s", i, track.ID(), track.Kind().String())
			}
	...
	pc.OnTrack(func(track *webrtc.TrackRemote, receiver *webrtc.RTPReceiver) {
		c.logFn("vk: remote track: %s id=%s stream=%s", track.Codec().MimeType, track.ID(), track.StreamID())
...
func (c *VKClient) readTrack(track *webrtc.TrackRemote) {
	...
	ReadTrack(track, claim, release, c.logFn, "vk")
}
