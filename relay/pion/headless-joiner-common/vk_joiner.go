...
func (h *VKHeadlessJoiner) initPC() {
	...
	if mode == "video" {
		...
		startRTCPFeedbackReaders(pc, h.sampleTracks, h.vp8tunnels, h.bondedTunnel, h.logFn, "headless")
	}
	...
	pc.OnConnectionStateChange(func(state webrtc.PeerConnectionState) {
		...
		if state == webrtc.PeerConnectionStateDisconnected || state == webrtc.PeerConnectionStateFailed {
			if h.laneBinder != nil {
				h.logFn("headless: resetting lane binder due to state=%s", state.String())
				h.laneBinder.Reset()
			}
		}
		...
		if mode == "video" && state == webrtc.PeerConnectionStateConnected && len(h.vp8tunnels) > 0 && !h.tunnelStarted {
	...
	if mode == "video" {
		pc.OnTrack(func(track *webrtc.TrackRemote, receiver *webrtc.RTPReceiver) {
			h.logFn("headless: remote track: %s id=%s stream=%s", track.Codec().MimeType, track.ID(), track.StreamID())
			var claim func(*webrtc.TrackRemote) *tunnel.VP8DataTunnel
			var release func(*webrtc.TrackRemote)
			if h.laneBinder != nil {
				claim = h.laneBinder.Claim
				release = h.laneBinder.Release
			}
			go h.ReadTrackFn(track, claim, release, h.logFn, "headless")
		})
	}
