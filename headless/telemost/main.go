...
type guestStatusEmitter struct{}

func (guestStatusEmitter) EmitStatus(status string) {
	log.Printf("[guest] status: %s", status)
}

func (guestStatusEmitter) EmitStatusError(msg string) {
	log.Printf("[guest] ERROR: %s", msg)
}
...
func runGuestCreator(joinLink string, displayName string, readBuf int) {
	trimmedLink := strings.TrimSpace(joinLink)
	if trimmedLink == "" {
		log.Fatal("Guest mode requires --join-link")
	}
	...
	params, err := json.Marshal(struct {
		...
		JoinLink:    trimmedLink,
		DisplayName: displayName,
	})
	if err != nil {
		log.Fatalf("[guest] marshal params: %v", err)
	}

	log.Printf("[guest] Starting Telemost guest creator for %s as %q", trimmedLink, displayName)
	guestJoiner := joiner.NewTelemostHeadlessJoiner(log.Printf, resolveHost, guestStatusEmitter{}, nil, pion.AddTelemostTunnelTracks, pion.ReadTrack)
	guestJoiner.OnConnected = func(tun tunnel.DataTunnel) {
		tunnel.NewRelayBridge(tun, "creator", readBuf, log.Printf)
		fmt.Print("\n  TUNNEL CONNECTED\n")
	}
	guestJoiner.RunWithParams(string(params))
}
...
func main() {
	...
	if strings.TrimSpace(*joinLink) != "" {
		if *writeFile != "" {
			...
			fmt.Fprintln(f, strings.TrimSpace(*joinLink))
			f.Close()
			log.Printf("[config] Wrote join link to %s", *writeFile)
		}
		runGuestCreator(*joinLink, *displayName, readBuf)
		return
	}
