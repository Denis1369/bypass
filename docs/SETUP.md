...
Для **Telemost** есть и второй вариант: можно создать конференцию где угодно, скопировать ссылку и передать на сервер только `join link`. В этом режиме сервер зайдет как гость и куки Yandex на VPS не нужны.
...
# Telemost: create a new room from an authenticated account
./headless-telemost-creator --cookies cookies-yandex.json

# Telemost: join an existing room as a guest (no cookies on VPS)
./headless-telemost-creator --join-link "https://telemost.yandex.ru/j/..." --display-name "Server"
```

В режиме с куками Creator создаст звонок и выведет ссылку в лог. Ссылку нужно передать на Joiner.

В режиме `--join-link` сервер не создает звонок сам, а заходит в уже существующую Telemost-конференцию как гость. Это удобнее и безопаснее, если не хочется держать Yandex cookies на VPS.

### Docker на VPS

Если удобнее запускать серверную часть в Docker:

```sh
./docker-vps/install-docker-ubuntu24.sh

cp docker-vps/.env.example docker-vps/.env
# укажите TELEMOST_JOIN_LINK и при желании TELEMOST_DISPLAY_NAME
...
Этот контейнер предназначен именно для Telemost guest-режима по `join link`. Порты пробрасывать не нужно: creator-сторона сама устанавливает исходящие соединения, а весь трафик и логи остаются внутри контейнера.
...
| `--join-link <url>` | Ссылка на уже созданную Telemost-конференцию; сервер зайдет как гость |
| `--display-name <name>` | Имя гостя для режима `--join-link` |
